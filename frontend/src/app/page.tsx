...
export default function Home() {
  ...
  }, [autoUpdate, updateFrequency, ticker, timeframe, emaPeriod, vwapPeriod, vwapStdDev]);

  const fetchData = async (ticker: string, timeframe: string, emaPeriod: number, vwapPeriod: number, vwapStdDev: number) => {
    try {
      const response = await fetch(`http://localhost:5001/api/data/${ticker}/${timeframe}/${emaPeriod}/${vwapPeriod}/${vwapStdDev}`);
  ...
  return (
    <div className={`flex h-screen ${theme === 'light' ? 'bg-white text-black' : 'bg-gray-900 text-white'}`}>
      <div className="flex-grow flex flex-col">
        <header className="p-4 bg-gray-800 text-white">
          <div className="flex justify-between items-center mt-4">
            <div className="flex space-x-4 items-center">
              ...
              />
              <button
                onClick={() => fetchData(ticker, timeframe, emaPeriod, vwapPeriod, vwapStdDev)}
        ...
        <ResizablePanelGroup direction="horizontal" className="flex-grow">
          ...
          <div className={`watchlist p-3 h-full overflow-y-auto ${theme === 'light' ? 'bg-gray-100 text-black' : 'bg-gray-800 text-white'}`}>
            ...
            <div className="flex flex-col items-center">
              {watchlist.length > 0 ? (
                watchlist.map((item) => (
                  <div
                    key={item.symbol}
                    className={`watchlist-item ${theme === 'light' ? 'watchlist-item-light' : 'watchlist-item-dark'}`}
                    onClick={() => {
                      setTicker(item.symbol);
                      fetchData(item.symbol, timeframe, emaPeriod, vwapPeriod, vwapStdDev);
                    }}
                  >
                    {item.symbol}
                  </div>
          ...
          <ResizablePanel>
            <div className="flex h-full">
              ...
              <div className={`active-trader w-1/4 p-4 flex-grow ${theme === 'light' ? 'active-trader-light' : 'active-trader-dark'}`}>
